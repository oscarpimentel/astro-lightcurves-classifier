...
def get_ps_performance_df(rootdir, cfilename, kf, set_name, model_names, metric_names,
    ...
    if baseline_d is not None:
        new_model_names = list(baseline_d.keys())
        is_baseline_lst = [True] * len(new_model_names)
    new_model_names += utils.get_sorted_model_names(model_names)
    is_baseline_lst += [False] * len(model_names)
    for kmn, (model_name, is_baseline) in enumerate(zip(new_model_names, is_baseline_lst)):
        if is_baseline:
            load_roodir = baseline_d[model_name]['path']
        else:
