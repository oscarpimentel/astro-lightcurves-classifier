...
from fuzzytorch.models.others import TimeFILM

###################################################################################################################################################

class TimeModRNNEncoderS(nn.Module):
    def __init__(self,
        **kwargs):
        super().__init__()
        ### attributes
    ...
    def reset(self):
        ...
        ### time_film
        self.time_film = TimeFILM(self.embd_dims, self.embd_dims, self.max_period,
            kernel_size=self.kernel_size,
            time_noise_window=self.time_noise_window,
            )
        print(f'self.time_film={self.time_film}')
    ...
    def forward(self, tdict:dict, **kwargs):
        ...
        encz = self.time_film(encz, rtime, onehot) # (n,t,f)>(n,t,f)
...
class TimeModRNNEncoderP(nn.Module):
    ...
    def reset(self):
        ...
        ### time_film
        self.time_film = nn.ModuleDict({b:TimeFILM(band_embedding_dims, band_embedding_dims, self.max_period,
            kernel_size=self.kernel_size,
            time_noise_window=self.time_noise_window,
            ) for b in self.band_names})
        print(f'self.time_film={self.time_film}')
    ...
    def forward(self, tdict:dict, **kwargs):
        ...
        for kb,b in enumerate(self.band_names):
            ...
            p_encz = self.x_projection[b](torch.cat([p_x], dim=-1)) # (n,t,i)>(n,t,f)
            p_encz = self.time_film[b](p_encz, p_rtime, p_onehot) # (n,t,f)>(n,t,f)
