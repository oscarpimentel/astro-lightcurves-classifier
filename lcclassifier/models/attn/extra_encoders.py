...
from fuzzytorch.models.others import TemporalEncoder

ATTN_DROPOUT = _C.ATTN_DROPOUT

###################################################################################################################################################

class TimeCatSelfAttnEncoderS(nn.Module):
	def __init__(self,
		**kwargs):
		super().__init__()
		### attributes
	...
	def reset(self):
		...
		self.x_projection = Linear(self.input_dims+nof_bands+self.te_features, self.embd_dims, # input+bands+te (i+b+2M,f)
			activation='linear',
			bias=False,
			)
		...
		### temporal_encoder
		self.temporal_encoder = TemporalEncoder(self.te_features, self.max_period,
			time_noise_window=0,
			init_k_exp=0,
			)
		print(f'temporal_encoder={self.temporal_encoder}')
		
		### attn
		self.ml_attn = ft_attn.MLSelfAttn(self.embd_dims, self.embd_dims, [self.embd_dims]*(self.layers-1),
	...
	def forward(self, tdict:dict, **kwargs):
		...
		te = self.temporal_encoder(rtime, onehot)
		encz = self.x_projection(torch.cat([x, s_onehot.float(), te], dim=-1)) # (n,t,i+b+2M)>(n,t,f)
		encz, scores = self.ml_attn(encz, onehot, return_only_actual_scores=True)
...
class TimeCatSelfAttnEncoderP(nn.Module):
	def __init__(self,
		**kwargs):
		super().__init__()
		### attributes
	...
	def reset(self):
		...
		self.x_projection = nn.ModuleDict({b:Linear(self.input_dims+self.te_features, band_embedding_dims, # input (i+2M,f/b)
			activation='linear',
			bias=False,
		...
		### temporal_encoder
		self.temporal_encoder = nn.ModuleDict({b:TemporalEncoder(self.te_features, self.max_period,
			time_noise_window=0,
			init_k_exp=0,
			) for b in self.band_names})
		print(f'temporal_encoder={self.temporal_encoder}')
		...
		### attn
		self.ml_attn = nn.ModuleDict({b:ft_attn.MLSelfAttn(band_embedding_dims, band_embedding_dims, [band_embedding_dims]*(self.layers-1),
	...
	def forward(self, tdict:dict, **kwargs):
		...
		for kb,b in enumerate(self.band_names):
			...
			p_te = self.temporal_encoder[b](p_rtime, p_onehot)
			print(torch.cat([p_x, p_te], dim=-1).shape)
			p_encz = self.x_projection[b](torch.cat([p_x, p_te], dim=-1)) # (n,t,i+2M)>(n,t,f)
			p_encz, p_scores = self.ml_attn[b](p_encz, p_onehot, return_only_actual_scores=True)
			if self.add_extra_return:
